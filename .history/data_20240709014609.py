...
def PredictValuesFor1Values(numberOfValuesToPredict, model: keras.Model, entries, timestep):
      ...
      predictionInput = values[-timestep:]
      predictionOutput = model(values)

def PredictValues2(numberOfValuesToPredict, model: keras.Model, entries, timestep):
      valuesPredicted = {}
      predictions = []
      predictionInput = np.reshape(list(entries.values())[-timestep:], (1, timestep, 1)) # change to (1, timestep, 1) for 1 value entries
