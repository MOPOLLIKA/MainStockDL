...
class ModelEnsemble(keras.Model):
      ...
      def __call__(self, inputs):
            ...
            modelBaseOutputNormalized = np.reshape(modelBaseOutput, (3, 1, 1)) # NormalizeOutput(modelBaseOutput, timestep=3)
            print("ensemble outputs:")
            print(modelOutput)
            print(modelBaseOutputNormalized)
            ensembleOutput = (modelOutput + modelBaseOutputNormalized) / 2
            print(ensembleOutput)
            return ensembleOutput
      
      def evaluate(self, x_train, y_train, batch_size, verbose):
            changes: list = []
            for index in range(len(x_train)):
...
def main() -> None:
      ...
      for ticker in stocksEntries:
            ...
            modelBase.fit(x_train, y_train, batch_size=1, epochs=10, validation_split=0.1, callbacks=earlyStopping, verbose=2)

            #earlyStopping = keras.callbacks.EarlyStopping("val_loss", patience=3, restore_best_weights=True)
            #model.fit(x_train, y_train, batch_size=1, epochs=10, validation_split=0.1, callbacks=earlyStopping, verbose=2)
            ...
            result = modelEnsemble.evaluate(x_train, y_train, batch_size=1, verbose=2)
