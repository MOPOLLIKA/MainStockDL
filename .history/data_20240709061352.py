...
def PlotPredictionGraphFor1Values(model: keras.Model, x_train, entries):
      ...
      for predictionInput in x_train:
            ...
            print()
            prediction = predictionOutput[0][-1]
            yPredicted.append(prediction)
      
      plt.plot(xActual, yActual, color="red")
      plt.plot(xPredicted, yPredicted, color="blue")
      plt.show()
      return True
...
def CreateLstmModel(inputShape, units):
      model = keras.Sequential()
      model.add(layers.LSTM(units, return_sequences=True, input_shape=inputShape))
...
if __name__ == "__main__":
      ...
      model = CreateLstmModel((timestep, numberOfValues), 50)
      earlyStopping = keras.callbacks.EarlyStopping("val_loss", patience=5, restore_best_weights=True)
      model.fit(x_train, y_train, batch_size=1, epochs=5, verbose=1, callbacks=earlyStopping, validation_split=0.1)
