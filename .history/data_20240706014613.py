...
def PredictValues2(number, model: keras.Model, entries, timestep):
      ...
      for _ in range(number):
            ...
            print(valuesPredicted)
            while not IsTradingOnDate(dateNext):
                  dateNext += timeDifference
            predictionInput = predictions[-1][0]
...
def CreateDatasetFromEntries(entries):
      ...
      for time in list(entries.keys())[1:]:
            entry = np.reshape(entries[time], (-1, 1)) # change to (-1, 3) when taking 3 values as inputs
...
def PlotPredictionGraph(model: keras.Model, x_train, entries):
      numberOfPoints = len(entries)
      xActual = range(1, numberOfPoints)
      yActual = list(entries.values())
      xPredicted = range(numberOfPoints)

      yPredicted = []
      predictionFirst = TensorToList(model(x_train[0]))
      yPredicted.append(np.reshape(predictionFirst, (1, -1))[0])
      for entryTrain in x_train[1:]:
            prediction = TensorToList(model(entryTrain))
            yPredicted.append(prediction[-1][0][0])

      for index in range(xPredicted):
            print(f"x{index}: {xPredicted[index]}, y{index}: {yPredicted[index]}")
...
if __name__ == "__main__":
      number = 3
      data = FetchDataYF(ticker="NVDA", interval="1d")
      indicators = FetchIndicators(ticker="NVDA", interval="DAILY")
      entries = TransformDataIntoSequence1(data, indicators)
      entriesTransformed = TransformEntries(entries, number, number)
      x_train, y_train = CreateDatasetFromEntries(entriesTransformed)

      model = CreateBasicModel((number, 1), 64)
      ...
      prediction = PredictValues1(number, model, entries)
      
      PlotPredictionGraph(model, x_train, entries)
