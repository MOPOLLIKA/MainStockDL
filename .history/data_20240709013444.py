...
def FetchIndicators(ticker="AAPL", interval="DAILY", series_type="close", apikey="U2NTP7T784ZP3OJK"):
      ...
      if name in os.listdir(path):
            with open(os.path.join(path, name), "r") as f:
                  ...
                  f.close()
                  if not "Error Message" in indicators.keys() and list(indicators.keys())[-1] in LastNumberWorkingDays(5):
...
def PredictValuesForAnyValues(numberOfValuesToPredict, model: keras.Model, entries, numberOfValues):
      """
      Predicts a certain number of candles ahead in the future using a model and entries data with clusters of . It returns {"date": value, "date": value, ...}
      """
      #entriesTransformed = TransformEntries(entries, number, number)
      valuesPredicted = {}
      predictionInput = np.reshape(list(entries.values())[-numberOfValuesToPredict:], (1, numberOfValuesToPredict, numberOfValues)) # risky move
...
def PredictValuesFor1Values(numberOfValuesToPredict, model: keras.Model, entries):
...
def GetStocksEntries(numberOfValues=3):
      ...
      try:
            indicators = FetchIndicators(ticker="ILMN")
      except:
...
def TrainBaseNumberModel(numberOfTickers):
      with open("/Users/MOPOLLIKA/python_StockDL/stockdata/stocksEntries.json", "r") as f:
            stocksEntries = json.load(f)
            f.close()
      timestep = 4
      model = CreateBasicModel((timestep, 3), 128)
      checkpoints = keras.callbacks.ModelCheckpoint(filepath=f"/Users/MOPOLLIKA/python_StockDL/ModelCheckpoints/base{numberOfTickers}.weights.h5", save_weights_only=True, verbose=1)
      ...
      for ticker in list(stocksEntries.keys())[0:numberOfTickers]:
            ...
            model.fit(x_train, y_train, batch_size=1, epochs=50, verbose=1, callbacks=[earlyStopping, checkpoints], validation_split=0.1)
      return model
...
def CreateModelBaseNumber(timestep, numberOfValues, numberOfTickers):
      path = f"/Users/MOPOLLIKA/python_StockDL/ModelCheckpoints/base{numberOfTickers}.weights.h5"
...
if __name__ == "__main__":
      ...
      numberOfTickers = 20
      entries = CreateEntries("AAPL", numberOfValues)
      modelBase = CreateModelBaseNumber(timestep, numberOfValues, numberOfTickers)
      prediction = PredictValuesForAnyValues(numberOfValuesToPredict, modelBase, entries, numberOfValues)
      print(prediction)
      ...
      for ticker in stocksEntries:
            ...
            modelBase20 = CreateBasicModel((timestep, numberOfValues), 128)
            modelBase20.load_weights("/Users/MOPOLLIKA/python_StockDL/ModelCheckpoints/base20.weights.h5")

            #earlyStopping = keras.callbacks.EarlyStopping("val_loss", patience=5, restore_best_weights=True)
            #modelBase20.fit(x_train, y_train, batch_size=1, epochs=50, callbacks=earlyStopping, validation_split=0.1, verbose=0)
            result = modelBase20.evaluate(x_train, y_train, batch_size=1, verbose=2)
