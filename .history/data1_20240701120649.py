...
def TransformDataIntoSequence1(data, indicators):
      ...
      standardized = StandardizeArraySK(array, 1000)
...
def StandardizeArraySK(array, nValues):
      length = len(array)
      numberOfChunks = length - nValues + 1
      arrayNew = []
      ...
      for index in range(nValues, numberOfChunks):
            chunk = array[index - nValues:index]
            scaler = StandardScaler().fit(chunk)
            standardized = scaler.transform(chunk)
            arrayNew += ToList(standardized)

      chunkLast = array[numberOfChunks * nValues:]
