...
Plans:
      ...
      CURRENT OBJECTIVES - Road to finishing the project!
      1. Collect the accuracy data for all the NASDAQ tickers using 3 values and 1 value entries with the basic model.
            ...
            Abandon the 3 value strategy and complete the 1 value prediction method, and plotting method to create required graphs of actual/predicted(covered+uncovered) data
      2. Create a uniform model which was trained on all the NASDAQ tickers and add its weight to the prediction of values in the main model architecture
      3. Start fetching real world data and reshaping it into model appropriate format to then do some learning and comparing of the effectiveness
      4. Learn github and push all of the files there to become open-source
      5. Draw conclusions from the work done

      MAIN THING
      To accomplish the task of determining whether there is something general in the randomness of natural phenomena and stock price movement a prediction function 
      should be composed based on an ensemble of the stock base model and specific data model and the results should be compared to those of just a single model for 
      the natural phenomena data. If the result is better, then there is some generality, absorbed by the stock model. Also, a second base model should be composed out
      of all the top 100 tickers on nasdaq. Both models should be included in the table and results compared. All of this is going to happen in 1 value case of data.
      In addition to the tables, there should be a plotting function made to plot the actual, predicted and uncovered predicted data and then after several days elapse 
      it should be compared and a new graph plotted with more actual data. For that there should be a file created to store the uncovered predictions and a function
      which checks, whether there are new actual data points intersecting with the data stored in the file.
      Then there is the write up part]]]]...

      TODO
      Create a comparison of a model with the random one. The random model is the one which doesn't change the input at all and leaves all as it is.
"""
