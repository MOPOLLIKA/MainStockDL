...
Plans:
      ...
      2. Fine-tune it
      3. Function predicting n values forward in format: {"future_date_1": value_1, "future_date_2": value_2, ... , "future_date_n": value_n}
      4. Reduce all OHLC values only to C and standardize them - COMPLETED
      5. Try overnight units - 512, timestep - 20, lr - 0.000001
      
      The data is very noisy which reduces the effectiveness of the model. The possible alterations to be made:
      1. Reduce the data. Leave only necessary indicators and try to combine them, so that only 3 values are left - price, volume and indicator feature, with input and output masks for all of them, also the output 
...
class StockModel(keras.Model):
      def __init__(self, inputSize, outputSize, timestep, units, unitsReduction, dropoutFr):
