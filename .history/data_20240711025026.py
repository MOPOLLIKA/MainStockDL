...
def TrainBaseNumberModel(numberOfTickers, numberOfValues, timestep, lstm=False):
      ...
      model = CreateBasicModel((timestep, numberOfValues), 128) if lstm == False else CreateLstmModel((timestep, numberOfValues), 128)
...
def CreateModelBaseNumberLstm(timestep, numberOfValues, numberOfTickers) -> keras.Model:
      path = f"/Users/MOPOLLIKA/python_StockDL/ModelCheckpoints/base{numberOfTickers}_{numberOfValues}_lstm.weights.h5"
      modelBase = CreateLstmModel((timestep, numberOfValues), 128)
      modelBase.load(path)
      return modelBase
...
if __name__ == "__main__":
      ...
      TrainBaseNumberModel(numberOfTickers, number)
