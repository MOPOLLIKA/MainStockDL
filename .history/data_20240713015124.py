...
def CreateEnsembleModel(modelBase: keras.Model, inputShape, units):
      model = keras.Sequential()
...
def main() -> None:
      ...
      modelBase = CreateLstmModel((timestep, numberOfValues), 64)# LoadModelBaseNumberLstm(timestep, numberOfValues, numberOfTickers)
      ...
      for ticker in stocksEntries:
            ...
            modelBase.fit(x_train, y_train, batch_size=1, epochs=10)
            modelBase.evaluate(x_train, y_train)
            print(modelBase(x_train[0]))
            print(y_train[0])
