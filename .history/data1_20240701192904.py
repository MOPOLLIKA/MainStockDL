...
def TimeDifference(entries):
      ...
      """
      datesLast = list(entries.keys())[-3:]
      datesLastDT = list(map(lambda x: dt.datetime.strptime(x, "%Y-%m-%d"), datesLast))
      datesDifference = [datesLastDT[index] - datesLastDT[index - 1] for index in range(len(datesLastDT))]
...
def PredictValues(number, model: keras.Model, entries):
      ...
      valuesPredicted = {}
      predictionInput = np.array(list(entriesTransformed.values())[-number:])
      predictionOutput = model(predictionInput)
      datesLast = list(entries.keys())[-number:]
      datesLastDT = list(map(lambda x: dt.datetime.strptime(x, "%Y-%m-%d"), datesLast))
...
def TransformDataIntoSequence1(data, indicators):
      ...
      indicatorsValues = {time: indicators[time][7] for time in data} # take only certain indicator values
...
if __name__ == "__main__":
      ...
      entriesTransformed = TransformEntries(entries, 3, 2)
      x_train, y_train = CreateDatasetFromEntries(entriesTransformed)
