...
def CreateDatasetFromEntries(entries):
      ...
      for time in list(entries.keys())[1:]:
            entry = np.reshape(entries[time], (-1, 3))quit
...
if __name__ == "__main__":
	...
	model2 = CreateConvolutionalModel((timestep, 3), 64, 5)
	model2.compile(
		optimizer=keras.optimizers.Adam(learning_rate=1e-3),
		loss=keras.losses.MeanSquaredError()
	)
	model2.summary()
	model2.fit(x_train, y_train, batch_size=4, epochs=30, verbose=1, callbacks=keras.callbacks.EarlyStopping("val_loss", patience=5, restore_best_weights=True), validation_split=0.1)
	TestModel(model2, x_train, y_train)
