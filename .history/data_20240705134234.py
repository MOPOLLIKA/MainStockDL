...
def LastThreeWorkingDays():
      ...
      while not IsTradingOnDate(date):
            date -= timeDelta
      return [date, date - timeDelta, date - (2 * timeDelta)]
...
def PredictValues2(number, model: keras.Model, entries, timestep):
      valuesPredicted = {}
      predictions = []
      predictionInput = np.reshape(list(entries.values())[-timestep:], (1, timestep, 3))
      timeDifference = TimeDifference(entries)
      ...
      for _ in range(number):
            predictionOutput = model(predictionInput)
            prediction = TensorToList(predictionOutput)[0]
            predictions.append(prediction)
            print(predictions)
            valuesPredicted[str(dateNext.date())] = prediction[0]
            dateNext = dateNext + timeDifference

            if len(predictions) >= timestep:
                  predictionInput = predictions[-timestep:]
                  continue
            numberOfValuesFromEntries = timestep - len(predictions)
            print(numberOfValuesFromEntries)
            print(predictions)
            #print(list(entries.values())[-numberOfValuesFromEntries:])
            predictionInput = np.append(list(entries.values())[-numberOfValuesFromEntries:], predictions)
...
def CreateDatasetFromEntries(entries, retainShape):
...
if __name__ == "__main__":
      timestep = 4
      number = 4
      data = FetchDataYF(ticker="AAPL", interval="1d")
      indicators = FetchIndicators(ticker="AAPL", interval="DAILY")
      entries = TransformDataIntoSequence1(data, indicators)
      entriesTransformed = TransformEntries(entries, timestep, 1)
      x_train, y_train = CreateDatasetFromEntries(entriesTransformed, retainShape=False)
      for index in range(len(x_train)):
            print(f"x_train{index}: {x_train[index]}, y_train{index}: {y_train}")


      model = CreateBasicModel((timestep, 3), 64)
      ...
      prediction = PredictValues2(number, model, entries, timestep)
      print(prediction)
