...
class ConvolutionalModel(keras.Model):
	def __init__(self, filters, kernelSize):
		super().__init__()
		self.conv1 = layers.Conv1D(filters, kernelSize)
		self.maxpooling1 = layers.MaxPool1D()
...
def CreateConvolutionalModel(inputSize, filters, kernelSize):
...
if __name__ == "__main__":
	...
	entriesTransformed = TransformEntries(entries, 200, 1)
	x_train, y_train = CreateDatasetFromEntries(entries)
	print(f"x_train1: {x_train[0]}, y_train1: {y_train[0]}")
	model2 = ConvolutionalModel(32, 3)
	model2.compile(
		optimizer=keras.optimizers.Adam(learning_rate=1e-3),
		loss=keras.losses.MeanSquaredError()
	)
	model2.summary()
	model2.fit(x_train, y_train, batch_size=4, epochs=30, verbose=1, callbacks=keras.callbacks.EarlyStopping("val_loss", patience=5), validation_split=0.1)
