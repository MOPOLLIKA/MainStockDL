...
def PlotPredictionGraph(model: keras.Model, x_train, y_train, entries, prediction):
      ...
      inputLeft = np.reshape(sequenceLeft, (1, timestep, 1))
      outputLeft = TensorToList(model(inputLeft))
      yPredicted += list(np.ravel(outputLeft)[-numberLeft:])
      
      xUncovered = range(numberOfPoints + 1, len(prediction) + numberOfPoints + 1)
      yUncovered = list(prediction.values())

      plt.plot(xActual, yActual, color="red")
      plt.plot(xPredicted, yPredicted, color="blue")
      plt.plot(xUncovered, yUncovered, color="green")
      plt.legend(["Actual", "Covered prediction", "Uncovered Prediction"], loc="upper left")
      plt.show()
