...
def TrainBaseNumberModel(numberOfTickers, numberOfValues):
      with open("/Users/MOPOLLIKA/python_StockDL/stockdata/stocksEntries.json", "r") as f:
            stocksEntries = json.load(f)
            f.close()
      timestep = 4
      model = CreateBasicModel((timestep, numberOfValues), 128)
      checkpoints = keras.callbacks.ModelCheckpoint(filepath=f"/Users/MOPOLLIKA/python_StockDL/ModelCheckpoints/base{numberOfTickers}{numberOfValues}.weights.h5", save_weights_only=True, verbose=1)
...
def CreateModelBaseNumber(timestep, numberOfValues, numberOfTickers):
      path = f"/Users/MOPOLLIKA/python_StockDL/ModelCheckpoints/base{numberOfTickers}{numberOfValues}.weights.h5"
