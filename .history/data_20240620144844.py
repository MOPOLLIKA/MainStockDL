...
def PredictEntries(model: keras.Model, n, commonTime, lastDateIndex, lastEntry, interval, timestep, stride):
  ...
  dates = [str((dateZero + daysPerCandle * index).date()) for index in range(1, n + 1)]
  print(dates)
  nextEntry = lastEntry
  i = 0
  while i <= 5 * n:
    ...
    for innerIndex in range(timestep):
      ...
      if date not in commonTime: 
        ...
        i += 1
  valuesPredicted = {date: valuesPredicted[date] for date in valuesPredicted if date in dates and IsTrading}
