...
def main() -> None:
      ...
      for ticker in stocksEntries:
            ...
            model.fit(x_train, y_train, batch_size=1, epochs=20, validation_split=0.1, callbacks=earlyStopping, verbose=2)

            modelEnsemble = ModelEnsemble(model, modelBase)
            print(f"Model error: {model.evaluate(x_train, y_train, batch_size=1, verbose=0)}, modelBase error: {modelBase.evaluate(x_train, y_train)}")
