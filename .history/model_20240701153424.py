...
Plans:
      ...
      2. Fine-tune it
      3. Function predicting n values forward in format: {"future_date_1": value_1, "future_date_2": value_2, ... , "future_date_n": value_n} NEXT TARGET
      ...
      and output masks for all of them, also the output values s
      2. Timestep + stride strategy seems good
      3. Use practical component analysis to help the prediction
      4. Try using default scaler from scypy.preprocessing.StandardScaler, or rethink the standardization process
      5. 
      ...
      1. Parse data into sequence of shape=(3) vectors
      2. Split into slices with (sliceLength) length
      3. Attach the slice values to x_train and close values to y_train
      That was the current method, here is another one:
      1. Pass the whole sequence to the model
      2. ConvNet it.

      Create a dataset in a form of JSON file of the data for all tickers from nasdaq and also try other stock exchanges. The structure would be {"ticker": {"data": ..., "metadata:}}
