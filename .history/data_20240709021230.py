...
def CreateDatasetFromEntries(entries):
      ...
      for time in list(entries.keys())[1:]:
            ...
            y_train.append([[array[0]] for array in entry]) # made a change fromy_train.append([[array[0]] for array in entry])
...
def TrainBaseNumberModel(numberOfTickers, numberOfValues, timestep):
      with open("/Users/MOPOLLIKA/python_StockDL/stockdata/stocksEntries.json", "r") as f:
            stocksEntries = json.load(f)
            f.close()
      model = CreateBasicModel((timestep, numberOfValues), 128)
      checkpoints = keras.callbacks.ModelCheckpoint(filepath=f"/Users/MOPOLLIKA/python_StockDL/ModelCheckpoints/base{numberOfTickers}_{numberOfValues}.weights.h5", save_weights_only=True, verbose=1)
...
if __name__ == "__main__":
      ...
      numberOfTickers = 20
      modelBase20_1 = TrainBaseNumberModel(numberOfTickers, numberOfValues, timestep)
      sys.exit()
      entries = CreateEntries("AAPL", numberOfValues)
      modelBase = CreateModelBaseNumber(timestep, numberOfValues, numberOfTickers)
      prediction = PredictValuesFor1Values(numberOfValuesToPredict, modelBase, entries, timestep)
      print(prediction)
