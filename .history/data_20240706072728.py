...
def PredictValues1(number, model: keras.Model, entries):
      ...
      valuesPredicted = {}
      predictionInput = np.reshape(list(entries.values())[-number:], (1, number, 1))
...
def PredictValues2(number, model: keras.Model, entries, timestep):
      valuesPredicted = {}
      predictions = []
      predictionInput = np.reshape(list(entries.values())[-timestep:], (1, timestep, 1)) # change to (1, timestep, 3) for 3 value entries
      timeDifference = TimeDifference(entries)
...
def CreateBasicModel(inputShape, units, save=False):
      ...
      cpCallback = keras.callbacks.ModelCheckpoint(filepath="/Users/MOPOLLIKA/python_StockDL/ModelCheckpoints", save_weights_only=True, verbose=1) if save else []
      model.compile(
...
def Main():
      ...
      prediction = PredictValues2(number, model, entries, number)
      print(prediction)
...
if __name__ == "__main__":
      ...
      with open("/Users/MOPOLLIKA/python_StockDL/stockdata/stocksEntries.json", "r") as f:
            ...
            f.close()
      timestep = 4
      model = CreateBasicModel((timestep, 3), 128, save=True)
