...
if __name__ == "__main__":
      ...
      cpCallback = keras.callbacks.ModelCheckpoint(filepath="/Users/MOPOLLIKA/python_StockDL/ModelCheckpoints/model1.weights.h5", save_weights_only=True, verbose=1)
      for ticker in stocksEntries:
            entries = stocksEntries[ticker]
            entriesTransformed = TransformEntries(entries, timestep, timestep)
            x_train, y_train = CreateDatasetFromEntries(entriesTransformed)
            model.fit(x_train, y_train, batch_size=1, epochs=50, verbose=1, callbacks=[earlyStopping, cpCallback], validation_split=0.1)
