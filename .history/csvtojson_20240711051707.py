...
def AdjustForYears(entries: list[float]) -> list[float]:
      values: list = list(entries.values())
      keys: list = list(entries.keys())
      period: int = 12
      values: list[float] = [np.mean(values[index:index + period]) for index in range(0, len(values) - period, period)]
      entries: dict = []
      for key in keys:
            entries.append()
      return entries
...
if __name__ == "__main__":
      entries: list[dict] = CsvToEntries("/Users/MOPOLLIKA/python_StockDL/globaltemperature/GlobalLandTemperatures_GlobalTemperatures.csv")
