...
def FetchDataYF(ticker="TSLA", interval="1m"):
  """
...
def FetchIndicators(ticker, interval, series_type="close", apikey="U2NTP7T784ZP3OJK"):
  ...
  obv = unpackJSONIntoValues(requests.get(f"https://www.alphavantage.co/query?function=OBV&symbol={ticker}&interval={interval}&apikey={apikey}").json(), "OBV")
  ...
  adx = unpackJSONIntoValues(requests.get(f"https://www.alphavantage.co/query?function=ADX&symbol={ticker}&interval={interval}&time_period=14&apikey={apikey}").json(), "ADX")
  aroon = unpackJSONIntoValues(requests.get(f"https://www.alphavantage.co/query?function=AROON&symbol={ticker}&interval={interval}&time_period=25&apikey={apikey}").json(), "AROON")
  rsi = unpackJSONIntoValues(requests.get(f"https://www.alphavantage.co/query?function=RSI&symbol={ticker}&interval={interval}&time_period=14&series_type={series_type}&apikey={apikey}").json(), "RSI")
  stoch = unpackJSONIntoValues(requests.get(f"https://www.alphavantage.co/query?function=STOCH&symbol={ticker}&interval={interval}&apikey={apikey}").json(), "STOCH")
  cci = unpackJSONIntoValues(requests.get(f"https://www.alphavantage.co/query?function=CCI&symbol={ticker}&interval={interval}&time_period=20&apikey={apikey}").json(), "CCI")
  ...
  macd = {f"{time}": [(float(ema26JSON["Technical Analysis: EMA"][time]["EMA"]) - float(ema12JSON["Technical Analysis: EMA"][time]["EMA"]))] for time in sorted(ema26JSON["Technical Analysis: EMA"])}
...
def PredictEntries(model: keras.Model, n, commonTime, lastDateIndex, lastEntry, interval, timestep, stride):
  ...
  lastDate = commonTime[lastDateIndex]
  dateZero = dt.datetime.strptime(lastDate, "%Y-%m-%d")
  dates = [str((dateZero + daysPerCandle * index).date()) for index in range(n)]
  ...
  while i <= 5 * n:
    ...
    for innerIndex in range(timestep):
      ...
      if date not in commonTime: 
        ...
        i += 1
  valuesPredicted = {date: valuesPredicted[date] for date in valuesPredicted if key in dates}
