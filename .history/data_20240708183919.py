...
def SaveStocksEntries(stocksEntries: dict):
...
def GetStocksEntries():
      ...
      for ticker in tickers:
            ...
            try:
                  ...
                  print(f"Number of indicator requests has exceeded the limit of AlphaVantage. The last ticker loaded is: {list(stocksEntries.keys())[-1]}.")
                  return stocksEntries
            entries = TransformDataIntoSequence1(data, indicators)
            stocksEntries[ticker] = entries
            print(f"Fetched entries for ticker: {ticker}")
