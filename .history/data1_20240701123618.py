...
def TransformDataIntoSequence1(data, indicators):
      ...
      volumesListed = list(map(lambda x: list(x), list(np.reshape(list(volumes.values()), (-1, 1)))))
      volumesStandardized = StandardizeArraySK(volumesListed, 500)
      indicators = {time: indicators[time][7] for time in data} # take only obv values
      indicatorsListed = list(map(lambda x: list(x), list(np.reshape(list(indicators.values()), (-1, 1)))))
      indicatorsStandardized = StandardizeArraySK(indicatorsListed, 500)
      entries = {time: [closes[time], volumes[time], indicators[time]] for time in data}

      indicators
      if not len(volumes) == len(volumesStandardized):
            print(f"len(array): {len(volumes)}, len(standarsized): {len(standardized)}")
            raise ValueError("PIZDEC")
      for i in range(len(standardized)):
            print(f"Value_{i}: {array[i]}, standardized_{i}: {standardized[i]}")
      return entries
